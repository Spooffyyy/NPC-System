...
class NPCBuilder
{
    ...
    public function build(): CustomNPC
    {
        if ($this->world instanceof World && $this->position instanceof Vector3 && $this->name !== null && $this->name !== "") {
            ...
            $skinTag->setTag(new ByteArrayTag("CapeData", $skin->getCapeData()));
            $skinTag->setTag(new StringTag("GeometryName", $skin->getGeometryName()));
            $skinTag->setTag(new ByteArrayTag("GeometryData", $skin->getGeometryData()));
            $nbt->setTag($skinTag);
